...
class ModelArguments:
    model_name_or_path: str = field(metadata={"help": "모델 경로 또는 Hugging Face 모델 식별자"})
...
class DataTrainingArguments:
    ...
    instruction_text: str = field(default="", metadata={"help": "Source 텍스트에 추가할 Instruction (선택 사항)"})
...
# --- 모델 및 풀링 모듈 ---
class KUREModelWrapper(nn.Module):
    ...
    def _cls_pool(self, last_hidden_states: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, input_ids, attention_mask):
        ...
        cls_embedding = self._cls_pool(outputs.last_hidden_state)
...
class TripletDataCollatorForKURE:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Dict[str, torch.Tensor]]:
        if self.instruction:
            source_texts = [f"{self.instruction} {ex[self.source_col]}" for ex in features]
        else:
...
# --- 커스텀 트레이너 ---
class KURETripletTrainer(Trainer):
...
def main():
    ...
    model = KUREModelWrapper(model_args.model_name_or_path, model_args)

    train_dataset = TripletDataset(data_args)
    data_collator = TripletDataCollatorForKURE(tokenizer=tokenizer, data_args=data_args)

    trainer = KURETripletTrainer(
        model=model,
        args=training_args,
